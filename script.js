...
if (typeof setup === 'undefined' && typeof p5 !== 'undefined') {
    ...
    let particles = [];
    const NUM_PARTICLES = 60; // Fixed number of particles
    ...
    window.setup = function() {
        ...
        for (let i = 0; i < NUM_PARTICLES; i++) {
            particles.push({
                x: random(width),
                y: random(height),
                vx: random(-0.8, 0.8) || 0.5, // Prevent zero velocity
                vy: random(-0.8, 0.8) || 0.5,
                size: random(3, 5)
            });
        }
        
        frameRate(30); // Limit framerate to reduce CPU/GPU usage
    }

    window.draw = function() {
        if (!particleContainer) return;
        
        clear(); // Transparent background each frame (better performance, no black fading issue)
        
        const isLight = document.body.classList.contains('light-theme');
        const particleColor = isLight ? color(124, 58, 237, 100) : color(167, 139, 250, 100);
        ...
        for (let i = 0; i < particles.length; i++) {
            ...
            // Wrap around the screen to keep particles alive infinitely
            if (p.x < 0) p.x = width;
            if (p.x > width) p.x = 0;
            if (p.y < 0) p.y = height;
            if (p.y > height) p.y = 0;
